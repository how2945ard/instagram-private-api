...
Request.prototype.setMethod = function (method) {
  ...
  if (!_.includes(['POST', 'GET', 'PATCH', 'PUT', 'DELETE'], method)) throw new Error(`Method \`${method}\` is not valid method`);
...
Request.prototype.setBodyType = function (type) {
  if (!_.includes(['form', 'formData', 'json', 'body'], type)) throw new Error('`bodyType` param must be and form, formData, json or body');
...
Request.prototype.signData = function () {
  const that = this;
  if (!_.includes(['POST', 'PUT', 'PATCH', 'DELETE'], this._request.method)) throw new Error('Wrong request method for signing data!');
...
Request.prototype.parseMiddleware = function (response) {
  if (response.req._headers.host === 'upload.instagram.com' && response.statusCode === 201) {
    ...
    response.body = {
      status: 'ok', start: loaded[1], end: loaded[2], total: loaded[3],
    };
    return response;
  }
  try {
...
Request.prototype.errorMiddleware = function (response) {
  ...
  if (json.message == 'login_required') throw new Exceptions.AuthenticationError('Login required to process this request');
  if (json.error_type == 'sentry_block') throw new Exceptions.SentryBlockError(json);
  if (response.statusCode === 429 || _.isString(json.message) && json.message.toLowerCase().indexOf('too many requests') !== -1) throw new Exceptions.RequestsLimitError();
  if (_.isString(json.message) && json.message.toLowerCase().indexOf('not authorized to view user') !== -1) throw new Exceptions.PrivateUserError();
...
Request.prototype.send = function (options, attemps) {
  ...
  return this._mergeOptions(options)
    ...
    .then((response) => {
      ...
      if (_.isString(json.message) && json.message.toLowerCase().indexOf('transcode timeout') !== -1) throw new Exceptions.TranscodeTimeoutError();
      throw new Exceptions.RequestError(json);
    })
    .catch(error => that.beforeError(error, options, attemps))
    .catch((err) => {
    ...
    .catch(error => that.afterError(error, options, attemps));
};
