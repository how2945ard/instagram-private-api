      ...
      return request
        ...
        .then((uploadData) => {
          ...
          return Promise.mapSeries(chunks, (chunk, i) => _sendChunkedRequest(session, uploadData.params.uploadUrl, uploadData.params.uploadJob, sessionId, chunk.data, chunk.range, isSidecar))
            ...
            .then(uploadData => Upload.photo(session, photoStreamOrPath, uploadData.uploadId, 'cover_photo_', isSidecar)
              .then(() => uploadData));
        });
    });
};
    ...
    if (media.type === 'photo') {
      uploadPromises.push(
        Upload.photo(session, media.data, undefined, undefined, true)
          .then(payload => Promise.resolve(Object.assign({}, { uploadId: payload.params.uploadId }, media))),
      );
    }
    if (media.type === 'video') {
      uploadPromises.push(
        Upload.video(session, media.data, media.thumbnail, true)
          .then(payload => Promise.resolve(Object.assign({}, payload, media))),
      );
    }
  });
