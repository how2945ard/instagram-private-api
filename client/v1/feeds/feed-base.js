...
FeedBase.prototype.all = function (parameters) {
  ...
  _.defaults(parameters, {
    delay: 1500, every: 200, pause: 30000, maxErrors: 9, limit: this.limit,
  });
  ...
  // If ParseError, we assume that this is 403 forbidden HTML page, caused by "Too many requests". Just take a pause and retry.
    ...
    .then((response) => {
      const results = response.map(that.map);
      ...
      if ((parameters.limit && that.totalCollected > parameters.limit) || that._stopAll === true) exceedLimit = true;
